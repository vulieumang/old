 ...
 * $Id: blogger.php 1838 2011-03-07 16:26:27Z emloog $
 */
...
if ($action == '') {
	...
	if ($photo && file_exists($photo)){
		...
		$icon = "<img src=\"{$photo}\" width=\"{$imgsize['w']}\" height=\"{$imgsize['h']}\" border=\"1\" /><a href=\"javascript: em_confirm(0, 'avatar');\">[Xóa ảnh]</a>";
	}
...
if ($action == 'update_pwd') {
	...
	if(!$ispass)
	{
		formMsg('Sai mật khẩu hiện tại!','javascript:history.go(-1);',0);
	}elseif(!empty($login) && $User_Model->isUserExist($login, UID)){
		formMsg('Tên người dùng đã tồn tại!','javascript:history.go(-1);',0);
	}elseif(strlen($newpass)>0 && strlen($newpass) < 6){
		formMsg('Mật khẩu từ 6 ký tự trở lên','javascript:history.go(-1);',0);
	}elseif(!empty($newpass) && $newpass != $repeatpass){
		formMsg('Hai mật khẩu không giống nhau','javascript:history.go(-1);',0);
	}

	if(!empty($newpass) && empty($login))// Chi doi mat khau
	{
		$newpass = $PHPASS->HashPassword($newpass);
		$User_Model->updateUser(array('password'=>$newpass), UID);
		formMsg('Thay đổi mật khẩu thành công!','./',1);
	}elseif(!empty($newpass) && !empty($login))// Thay doi mat khau va tai khoan
	{
		$newpass = $PHPASS->HashPassword($newpass);
		$User_Model->updateUser(array('username'=>$login, 'password'=>$newpass), UID);
		formMsg('Thay đổi thành công tài khoản và mật khẩu! Xin vui lòng đăng nhập lại!','./',1);
	}elseif(empty($newpass) && !empty($login))// Chi doi ten dang nhap
	{
		$User_Model->updateUser(array('username'=>$login), UID);
		formMsg('Tên đăng nhập đã được thay đổi thành công! Xin vui lòng đăng nhập lại!','./',1);
	}else{
		formMsg('Vui lòng nhập thông tin muốn thay đổi!','javascript:history.go(-1);',0);
	}
}
