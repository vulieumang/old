<?php
/**
 * uu bai viet (bo sung, sua doi)
 * @copyright (c) Emlog All Rights Reserved
 * $Id: save_log.php 1894 2011-04-04 14:34:34Z emloog $
 */
...
$date = isset($_POST['date']) ? addslashes($_POST['date']) : '';// Thoi gian truoc khi sua
...
$blogid = isset($_POST['as_logid']) ? intval(trim($_POST['as_logid'])) : -1;// Tu dong luu ban nhap neu co ID
...
if($blogid > 0) {// Tu dong luu ban nhap, them thay doi de cap nhat
	...
	case 'edit':
		...
		if($ishide == 'y') {
			...
			if ($action == 'add' || isset($_POST['pubdf'])) {
				header("Location: ./admin_log.php?active_post=true");// Dang bai thanh cong
			} else {
				header("Location: ./admin_log.php?active_savelog=true");// Luu bai thanh cong
			}
		}
		break;
}
