 ...
 * $Id: xmlrpc.php 1908 2011-04-09 14:52:56Z colt.hawkins $
 */
...
// Xu ly cookie cua khac
$_COOKIE = array();
// Sua loi ham $HTTP_RAW_POST_DATA
...
if ($options_cache['isxmlrpcenable'] == 'n') {
	error_message(500, 'Chú ý:XMLRPC đóng.');
}
if (!$HTTP_RAW_POST_DATA) {
	error_message(500, 'Chú ý:XML-RPC chỉ chấp nhận POST.');
}
...
if (trim($data) == '') {
	error_message(500, 'Lỗi: Dữ liệu rỗng.');
}
// Fix loi phan tich cu phap xml, html, tag...
...
function mw_getPost($args) {
	...
	if (empty($post)) return error_message(404, 'Xin lỗi, bài viết không tồn tại!');
...
function mw_getRecentPosts($args) {
	...
	if (empty($xml)) {
		error_massage(404, 'Bài viết không tồn tại!!!');
	}
	$xml = "<array><data>$xml</data></array>";
	response($xml);
}
...
function mw_newMediaObject($args) {
	...
	if (empty($filename))
		error_message(500, 'Lỗi tên tập tin');

	$extension = strtolower(substr(strrchr($filename, "."), 1));
	// Kiem tra dinh dang
	if (!in_array($extension, $att_type)) {
		error_message(500, 'Lỗi định dạng tập tin');
	}
	$uppath_root = substr(Option::UPLOADFILE_PATH, 1);
	$uppath = $uppath_root . gmdate('mY') . '/';
	$fname = md5($filename) . gmdate('dmYHis') . '.' . $extension;
	$attachpath = $uppath . $fname;
	if (!is_dir($uppath_root)) {
		umask(0);
		$ret = @mkdir($uppath_root, 0777);
		if ($ret === false) {
			error_message(500, 'Khong the tai len tap tin.');
		}
	}
	if (!is_dir($uppath)) {
		umask(0);
		...
		if ($ret === false) {
			error_message(500, 'Tải lên không thành công. Thư mục(content/uploadfile) không thể ghi.');
		}
	}

	$fp = @fopen($attachpath, 'wb');
	if (!$fp)
		error_message(500, 'Không thể ghi');
	fwrite($fp, $bits);
	fclose($fp);
	...
	$img_url = $options_cache['blogurl'] . 'content/uploadfile/' . date('mY') . '/' . $fname;
...
function login($username, $password) {
	...
	if (!checkUser($username, $password , '')) {
		error_message(403, 'Tài khoản hoặc mật khẩu sai');
		return false;
	}
...
function tag_close($parser, $tag) {
	...
	switch ($tag) {
		...
		case 'dateTime.iso8601':
			...
			break;
		case 'value':
			// "Neu khong nhap, mac dinh kieu string"
