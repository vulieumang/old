...
jQuery(function($){$('#xiangcepeizhi').click(function(){location.href='./plugin.php?plugin=kl_album&kl_album_action=config'});$('#xinjianxiangce').click(function(){if(confirm('确定要建立一个新相册？')){$.get('../content/plugins/kl_album/kl_album_ajax_do.php?action=album_create&sid='+Math.random(),{is_create:'Y'},function(result){if($.trim(result).indexOf('kl_album_successed')!=-1){window.location.reload()}else{alert('发生错误:'+result)}})}})});
function album_getclick(num){$('input[name^=album_n_'+num+'],input[name^=album_d_'+num+'],input[name^=album_p_'+num+']').click(function(){$(this).removeClass('o_bg_color').addClass('no_bg_color');})};
...
function album_del(num){if(confirm('删除相册将一并删除该相册内所有相片，确定要删除？')){$.get('../content/plugins/kl_album/kl_album_ajax_do.php?action=album_del&sid='+Math.random(),{album:num},function(result){if($.trim(result).indexOf('kl_album_successed')!=-1){window.location.reload()}else{alert('发生错误:'+result)}})}}
...
<table height="100%" width="100%" border="0" style="border:1px solid #CCC;">
  <tr>
	<td width="110" height="110" rowspan="2" align="center"><a href="./plugin.php?plugin=kl_album&kl_album_action=display&album='.$val['addtime'].'">'.$kl_img_str.'</a></td>
	<td vlign="top">
	  <table border="0" width="100%" height="100%" style="border:1px solid #CCC;">
		<tr>
		  <td width="40" height="35"><nobr>相册名称：</nobr></td>
		  <td><input name="album_n_'.$key.'" type="text" value="'.$val['name'].'" class="o_bg_color" onclick="album_getclick('.$key.')" /></td>
		</tr>
		<tr>
		  <td height="35" <nobr>相册描述：</nobr></td>
		  <td><input name="album_d_'.$key.'" type="text" value="'.$val['description'].'" class="o_bg_color" onclick="album_getclick('.$key.')" /></td>
		</tr>
		<tr>
