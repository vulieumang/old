...
<?php
	...
	$log_photo_content = "";
	$log_photo_query = $DB->query("SELECT * FROM ".DB_PREFIX."options WHERE option_name='kl_album_info'");
	$log_photo_row = $DB->fetch_row($log_photo_query);
	if($log_photo_row)
	{
		$log_photo_info = unserialize($log_photo_row[2]);
	...
	foreach ($log_photo_info as $value)
	{
		...
		if($value['addtime'] == $album)
		{
			if($value['restrict'] == 'private' && ROLE != 'admin')
			{
				...
				$log_photo_query = $DB->query("SELECT * FROM ".DB_PREFIX."kl_album WHERE album=$album ORDER BY id DESC");
				while($log_photo = $DB->fetch_array($log_photo_query))
				{
					$log_photo_url = $blogurl.substr($log_photo['filename'],3);
					$log_photo_size = chImageSize(EMLOG_ROOT.str_replace('thum-', '',substr($log_photo['filename'],2)),480,360);
					$log_photo_content .= '
					<li id="attlist">
