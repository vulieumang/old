...
ogg_ok=false;mp3_ok=false; em=document.createElement('audio');
if(em.canPlayType && ''!=em.canPlayType('audio/mpeg')){mp3_ok=true;}
if(em.canPlayType && ''!=em.canPlayType('audio/ogg')){ogg_ok=true;}
...
if(ogg_ok || mp3_ok){
if(ogg_ok){x=skin_dir+'/sounds/o'+x+'.ogg';}
else if(mp3_ok){x=skin_dir+'/sounds/m'+x+'.mp3';}
...
if(r[3]){
if(r[1].length>0){document.getElementById('blab_onll').innerHTML=r[1];online_hash=r[3];if(!splay){splay=snd2;}}
...
function play_p(x){
close_l(); close_p(); set_opa('blab_ppnt1',1); x=parseInt(x); 
