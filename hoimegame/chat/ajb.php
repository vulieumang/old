...
$line=preg_replace("/\/p([0-9]+)/","<span class=\"blab_pt1\" onclick=\"play_p('$1')\"><img class=\"blab_pt2\" src=\"paintings/$1.png\" alt=\"\" /></span>",$line);}
$line=url2link($line);
...
if(strlen($row['from_name'])>0){$pm='<span class="blab_us" onclick="pat(\''.$row['from_name'].'\')">'.$row['from_name'].'</span>: ';}else{$pm='';}
if($format!='' && strlen($row['from_name'])>0){$tmm=gmdate($format,$row['timestamp']+$tzone*60);$tmm='['.$tmm.']';}else{$tmm='';}
$messages[]=$pnum.':|:<div><span class="s">'.$tmm.'</span> '.$pm.$row['line_txt'].'</div>';
