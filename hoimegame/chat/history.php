...
if($from<$lines_count){$inf_line='<div class="s">('.$lang_m.': '.$lines_count.') ('.$lang_p.': '.$pages_numbr.'/'.$pages_count.')</div>&nbsp;';print $inf_line;}
...
if(strlen($row['from_name'])>0){$pm='<b class="s">'.$row['from_name'].'</b>: ';}else{$pm='';}
$tmm=gmdate($format,$row['timestamp']+$tzone*60);$tmm='['.$tmm.']';
print '<div><span class="s">'.$tmm.'</span> '.$pm.$row['line_txt'].'</div>';}
